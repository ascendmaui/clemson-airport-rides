...
Auth matches the web `AuthProvider`: email and password via `signInWithPassword` / `signUp` on `https://awktabuhijrshmsmagpq.supabase.co`. Confirm-email is off on the project. Apple and Google sign-in are not used.
