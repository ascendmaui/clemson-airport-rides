...
export function collectTripPayment(body) {
  return api('/api/collect-payment', body)
}

export function settleTrip(body) {
  return api('/api/trip-settle', body)
}
...
export function buyCredits(tierId) {
  return api('/api/credits', { action: 'buy', tierId, nonce: String(Date.now()) })
}

export function retryDriverPayouts() {
  return api('/api/driver-payouts', {})
}
