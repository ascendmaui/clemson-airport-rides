...
test('complete uses unpaid fare; cancel uses only a precomputed fee', () => {
  ...
  const missingFare = amountDueForAction({ action: 'complete', fareCents: null, paidCents: 0 })
  assert.equal(missingFare.code, 'fare_not_set')
  assert.equal(missingFare.amountCents, null)
  const explicitZero = amountDueForAction({ action: 'complete', fareCents: null, explicitAmountCents: 0 })
  assert.equal(explicitZero.code, 'fare_not_set')
  const blankFare = amountDueForAction({ action: 'complete', fareCents: '' })
  assert.equal(blankFare.code, 'fare_not_set')
  const free = amountDueForAction({ action: 'complete', fareCents: 0, paidCents: 0 })
