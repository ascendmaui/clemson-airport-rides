...
import { applyRecoveryLink, currentRecoveryUrl } from '@/lib/recovery'
...
export default function SetPasswordRoute() {
  ...
  useEffect(() => {
    ...
    async function consume(url: string | null) {
      try {
        const result = url ? await applyRecoveryLink(url) : { applied: false as const }
        if (!alive) return
        if (result.applied) {
          setReady(true)
        ...
        setNote('Open the reset link from your email on this phone. It uses clemsonrides://set-password.')
      } catch (err) {
        if (!alive) return
