...
const DEFAULT_API_BASE = 'https://clemson-airport-rides.vercel.app'
...
test('non-OK JSON, malformed response, and network throw paths via apiErrorMessage', async () => {
  ...
  // Non-OK with neither error nor message falls back to `HTTP ${status}`
  stubFetch({
    status: 500,
    ok: false,
  ...
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.equal(apiErrorMessage(err), 'HTTP 503')
      return true
    },
  )

  // Invalid JSON (e.g. HTML 502 Bad Gateway) throws 'API unavailable'
  stubFetch({
    status: 502,
    ok: false,
  ...
  await assert.rejects(
    ...
    (err) => {
      ...
      assert.equal(apiErrorMessage(err), 'API unavailable')
      return true
    },
  )
