...
export function buyCreditPack(packId) {
  ...
  return api('/api/buy-credits', { method: 'POST', body: { packId, origin } })
}

export function confirmCreditPurchase(sessionId) {
  return api('/api/credits-confirm', { method: 'POST', body: { sessionId } })
}

export function startAirportCheckout(body) {
  const origin = typeof window !== 'undefined' ? window.location.origin : undefined
  return api('/api/airport-checkout', { method: 'POST', body: { ...body, origin } })
}
