# Ride with friends + Carpool — ship notes

## Payments (failure handling)
- Canonical collector: `collectPayment({ tripId, amountCents, methods: ['credits','card'] })` in `server/collectPayment.js` (re-exported from `server/friendRideLib.js`).
- 20% platform fee only: `shared/platformFee.js`. Wait-time and mid-ride cancel fees stay with their owners — pass `amountCents` or write `metadata.wait_fee_cents` / `metadata.cancel_fee_cents`. Do not fork that math.
- Trip complete/cancel that costs money goes through `POST /api/trip-settle`. `$0` proceeds. Otherwise status stays put and `metadata.payment_hold.status` is `payment_required`.
- Driver payout failures stay `metadata.payout.status = pending` and retry with backoff (`POST /api/driver-payouts`, cron when `CRON_SECRET` is set).
- Unpaid airport-deposit searching/offered/scheduled holds leave the pool 20 minutes after `created_at` or Checkout session bind (`metadata.stripe_checkout_created_at`, whichever is later). `GET` or `POST /api/expire-unpaid-airport-holds` is safe to call repeatedly from an external cron every 15 minutes (overlapping runs included). Send `Authorization: Bearer $CRON_SECRET` (trimmed; compared with `crypto.timingSafeEqual`). `x-vercel-cron: 1` is accepted only when `CRON_SECRET` is unset or a placeholder and `VERCEL` is set, so a public caller cannot spoof that header. Claiming uses a dedicated `trips.hold_expire_claimed_at` column (cleared on cancel or release), and cancel / checkout metadata writes use `public.merge_trip_metadata` to atomically merge into `trips.metadata` without whole-object overwrite (migration `supabase/migrations/20260925140000_hold_claim_atomic.sql` MUST be applied before shipping). The cancel matches only while status is still searching/offered/scheduled and `checkout_abandoned` is unset; only that winner writes `trip_events` and expires the Stripe Checkout session. Paid deposits, `deposit_cents = 0`, non-airport trips, and sessions Stripe reports as paid are skipped. A Stripe expire error is counted on that trip and the batch continues (40 holds max). `200` returns `{ ok, scanned, expired, released, skipped, errors }`. `401` if unauthorized, `405` for other methods, `503` if the service role client is missing, `500` with a generic error (detail is logged server-side). `?dry_run=1` sets `wouldExpire` and does not write. The cancel stamps `checkout_abandoned` the same way Checkout abandon does, so a later paid deposit restores the trip. Do not add a 15-minute cron to vercel.json; Hobby deploys reject that schedule. The 15-minute trigger is Supabase pg_cron job `expire-unpaid-airport-holds` (`7,22,37,52 * * * *`, migration `20260925160000_expire_holds_pg_cron.sql`), which calls the route via pg_net with the bearer from Vault secret `clemson_cron_secret`. That secret must equal Vercel `CRON_SECRET` (Production, Sensitive).
- Airport deposit reconciliation: Stripe webhook (`POST /api/stripe-webhook` handling `checkout.session.completed` / `async_payment_succeeded`) remains the primary path for recording paid deposits and restoring trips. On-demand reconciliation (`POST /api/stripe-payment-methods?action=reconcile-checkout` via `server/checkoutReconcile.js`) runs as a fallback when riders return from Stripe Checkout with a `session_id`, or if webhook delivery is delayed or misconfigured.

## Pre-ship database migrations (CRITICAL DEPLOY ORDER)
- **Unpaid Airport Hold Expiry Atomic Claims & Metadata Merge**
  - **Migration file:** `supabase/migrations/20260925140000_hold_claim_atomic.sql`
  - **Design chosen:** Combined Design A and Design B:
    - **Design A (Claim column):** Added dedicated `trips.hold_expire_claimed_at timestamptz` column (nullable) + partial index `trips_hold_expire_claimed_at_idx`. Claiming and releasing are single conditional UPDATEs on this column and never touch or read-modify-write `trips.metadata`.
    - **Design B (Atomic metadata merge):** Added PostgreSQL RPC `public.merge_trip_metadata(p_trip_id uuid, p_patch jsonb, ...)` (SECURITY DEFINER, `search_path = public`, granted to `service_role` only). Performs `metadata = coalesce(metadata, '{}'::jsonb) || p_patch` along with status/cancel updates in a single SQL operation. Eliminates the race condition where whole-object `trips.metadata` replacement clobbered concurrent Stripe webhook writes.
  - **DEPLOY ORDER (MUST RUN BEFORE SHIP):** Migration `supabase/migrations/20260925140000_hold_claim_atomic.sql` **MUST be applied to the database before this code ships**. If the application code is deployed before the migration is run, `/api/expire-unpaid-airport-holds` will fail with 500 errors because column `trips.hold_expire_claimed_at` and RPC function `public.merge_trip_metadata` will be missing.


## Payments (FINAL LOCK)
- SetupIntent save card (off_session)
- Organizer Confirm → off_session PI OR Payment Element / Apple Pay
- Full share; partial fail → retry; book only when all paid
- Do **not** invent Checkout pay-links for carpool (same as friends)

## Product — friends
1. /friends/:token (+ hash redirect)
2. Max 5; Routes optimize; automatic fare + surge
3. Fare split preview UI before charge
4. ride_bills itemized receipts
5. Ratings both sides; soft remind / skip
6. Multi-stop live tracking on Requested

## Product — carpool flagship

See `docs/CARPOOL_MATCHING.md` for the matching algorithm, the $30–$40 vs $10–$15 split, driver bonus id `driver_carpool_bonus`, and how this extends friend rides instead of forking them.

## Product — carpool (kind=carpool)
1. Organizer (student-with-car) creates → shareable `/carpool/:token`
2. Also accept `/friends/:token?kind=carpool`
3. Hash redirect: `#/carpool/:token` → `/carpool/:token`
4. Riders join with pickup/dropoff (max 5); reuse friend-ride APIs
5. Google Routes + even/by_distance + automatic fare (friendRideLib)
6. Charge: saved card off_session OR Apple Pay / Payment Element
7. All paid → one trips row; `driver_id` = organizer (skip open matching → status accepted)
8. Trust UI: "Clemson student" badge when `student_verified_at`; show ratings
9. Entry: Marketing / Friends / Account / DriverHome "Offer a carpool"

## Mid-ride cancel
- Status `canceled_midride` after pickup (`in_progress`, or `arriving` only if the trip already started). Pickup wait-fee cancel is a different flow.
- Obligation = min(quoted fare, max(meter, progress × quoted)) + cancel fee ($5 default, `MIDRIDE_CANCEL_FEE_CENTS`).
- Meter = $2.50 + $1.75/mi + $0.35/min. Progress = max(GPS distance / straight-line trip, elapsed / expected at 30 mph).
- Platform 20% / driver 80% of the obligation. Succeeded deposits reduce the new card charge only.
- Blocked after 3 mid-ride cancels in 30 days (`MIDRIDE_CANCEL_MAX`, `MIDRIDE_CANCEL_WINDOW_DAYS`).
- API: `POST /api/trip-cancel-midride` (`confirm: true` charges; omit confirm to preview).
- Card collection goes through `server/collectPayment.js` when that module is present. A decline still ends the trip and surfaces `payment_required` (toast + trip event). The trip is not left in progress.

## Payment kinds
Held SQL files each replace `payments_kind_check`. Apply `supabase/payments_kind_union.sql` last so tip, wait, cancel, mid-ride, and both credit purchase kinds can be inserted together.

## Env / blockers
- GOOGLE_MAPS_API_KEY (Vercel server Routes)
- VITE_GOOGLE_MAPS_API_KEY billed (Maps / CampusMap)
- Apple Pay domain in Stripe Dashboard
- sendmaui Maps billing if CampusMap blanks
