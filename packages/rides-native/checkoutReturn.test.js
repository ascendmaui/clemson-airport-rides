...
import { reconcileCheckout } from './riderMoney.js'
...
test('parseCheckoutSessionId: extracts session_id from web hash return URLs', () => {
  ...
  const fullLiveUrl = `https://clemson-airport-rides.vercel.app/${liveHash}`
  assert.equal(parseCheckoutSessionId(fullLiveUrl), 'cs_test_live123')

  const fullSchedUrl = `https://clemson-airport-rides.vercel.app/${schedHash}`
...
test('parseCheckoutSessionId: extracts session_id from query params in URL search', () => {
  const urlWithSearch = 'https://clemson-airport-rides.vercel.app/?session_id=cs_test_search789#/requested?trip=trip_123&paid=1'
...
test('parseCheckoutSessionId: returns null for missing or invalid inputs', () => {
  ...
  assert.equal(parseCheckoutSessionId('https://clemson-airport-rides.vercel.app/'), null)
  assert.equal(parseCheckoutSessionId({}), null)
...
test('parseCheckoutReturn: extracts full return context', () => {
  const schedReturn = parseCheckoutReturn(
    'https://clemson-airport-rides.vercel.app/#/schedule?paid=1&trip=trip_sched_100&session_id=cs_test_sched_ret'
  )
  ...
  const liveReturn = parseCheckoutReturn(
    'https://clemson-airport-rides.vercel.app/#/requested?trip=trip_live_200&paid=1&session_id=cs_test_live_ret'
  )
  ...
  const cancelReturn = parseCheckoutReturn(
    'https://clemson-airport-rides.vercel.app/#/schedule?canceled=1&trip=trip_cancel_300'
  )
