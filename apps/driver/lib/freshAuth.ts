...
// Driver copy of apps/rider/lib/freshAuth.ts (same guard; @clerk/expo resolves per app, so it cannot live in packages/rides-native).
...
/**
 * One guard for every driver auth entry point (Google, Apple, Facebook,
...
/**
 * The driver "/me": a Supabase session user plus a readable profiles row.
    ...
    const { error: insertError } = await supabase.from('profiles').upsert(
      {
        ...
        full_name: meta.full_name || meta.name || (user.email ? user.email.split('@')[0] : 'Driver'),
